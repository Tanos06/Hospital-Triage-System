...
bool validate_input_patient(const char* name, const char* surname,
                            ...
                            const char* deptId) {
    if(strlen(name) == 0 || strlen(surname) == 0 || strlen(taxCode) == 0 ||
       strlen(triage) == 0 || strlen(deptId) == 0) return false;
...
bool validate_parameters(const char* patient_email, const char* patient_name,
    ...
    if(strlen(patient_email) == 0 || strlen(patient_name) == 0 ||
       strlen(patient_surname) == 0 || strlen(diagnosis) == 0) return false;
    return true;
}
